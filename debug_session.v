...
Fail Timeout 5 Eval vm_compute in eval_with_fuel 3 one_plus_one.
(* = None
...
Goal eval_with_fuel 2 one_plus_one = Some two.
  ...
  unfold eval_with_fuel.
  (* You read the documentation of cbn and simpl, you can do the following *)
...
Goal eval_with_fuel 2 one_plus_one = Some two.
  ...
  Fail change (replace_fix1 one1 3 (var 3)) with one1. (* HUH *)
...
Fixpoint reduce_redexes_bottom_up_depfix1 t :=
  ...
  | appl (lambda t) x =>
    replace_fix1 (reduce_redexes_bottom_up_depfix1 x) O (reduce_redexes_bottom_up_depfix1 t)
  | appl t x =>
...
Fixpoint eval_with_fuel_depfix1 n t :=
  match n with
  | O => ensure_normal_form O t
  | S n => eval_with_fuel_depfix1 n (reduce_redexes_bottom_up_depfix1 t)
  end.
...
Goal eval_with_fuel_depfix1 2 one_plus_one = Some two.
  ...
  step match.
  do 2 step beta.
  change (reduce_redexes_bottom_up_depfix1 (var 0)) with (var 0).
  step zeta at 2.
  ...
  step match in e.
  step beta in e.
  change (reduce_redexes_bottom_up_depfix1 (var 1)) with (var 1) in e.
  change (reduce_redexes_bottom_up_depfix1 ?x) with x in e.
  change (lambda (appl (var 2) (var 0))) in (value of e).
  subst e.
  step match.
  step beta.
  Fail change (replace_fix1 ?x 0 (appl (var 2) (var 0))) with (appl (var 1) x). (* HUH *)
  step delta replace_fix1 at 2.
  step fix.
  fold replace_fix1.
  do 3 step beta.
  ...
  Fail change (replace_fix1 _ 0 (var 2)) with (var 1). (* HUH *)
  step delta replace_fix1 at 2.
  step fix.
  fold replace_fix1.
  ...
  set (
    fix replace_fix2 x k t :=
      ...
      | appl t u => appl (replace_fix2 x k t) (replace_fix2 x k u)
      end
  ) as replace_fix2.
  change_no_check replace_fix1 with replace_fix2.
  change (replace_fix2 ?x 0 (appl (var 2) (var 0))) with (appl (var 1) x).
  step zeta.
