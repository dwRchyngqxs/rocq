...
(* Arguments replace : simpl nomatch. *)

(* number one, reduce until no more originally visible redexes *)
Fixpoint reduce_visible_redexes t :=
  match t with
  | var n => var n
  | lambda t => lambda (reduce_visible_redexes t)
  | appl (lambda t) x => replace (reduce_visible_redexes x) O (reduce_visible_redexes t)
  | appl t x => appl (reduce_visible_redexes t) (reduce_visible_redexes x)
  end.
(* Arguments reduce_visible_redexes : simpl nomatch. *)
...
(* Arguments unlift : simpl nomatch. *)
...
(* Arguments ensure_normal_form : simpl nomatch. *)

(* call of nature, repeat number one *)
Fixpoint call_of_nature n t :=
  match n with
  | O => ensure_normal_form O t
  | S n => call_of_nature n (reduce_visible_redexes t)
  end.
(* Arguments call_of_nature : simpl nomatch. *)
...
*)
  ...
  step_in e0.
  set (e1 := ensure_normal_form 2 _) in e0.
  do 3 (step_in e1).
  set (e2 := ensure_normal_form 4 _) in e1.
  cbn [ensure_normal_form] in e2.
  (* if we get None at any point here, who can tell me why it appeared? *)
  Restart.
  
  unfold call_of_nature.
  cbn [reduce_visible_redexes one_plus_one].
  change (reduce_visible_redexes one) with one.
  change (reduce_visible_redexes one) with one. (* Why *)
  change (reduce_visible_redexes plus) with plus.
    unfold plus at 1.
  (* replace (reduce_visible_redexes one) 0
      (reduce_visible_redexes
         (lambda
  ...
  *)
  ...
  cbn [reduce_visible_redexes] in e.
  subst e.
    cbn [replace]. Undo.
  ...
  red_at (step delta) reduce_visible_redexes.
  red_at (step delta) reduce_visible_redexes.
Abort.
