...
let expand_case mind (ci, u, params, ((p, pt), rp), iv, c, br) =
  (* Γ ⊢ c : I@{u} params args *)
...
let rec subterm_specif ?evars renv stack t =
  ...
  let f,l = decompose_app_list (whd_all ?evars renv.env t) in
    ...
    | Case (ci, u, pms, p, iv, c, lbr) -> (* iv ignored: it's just a cache *)
      let _, (p, _), _, _, lbr =
        expand_case (lookup_mind_specif renv.env ci.ci_ind) (ci, u, pms, p, iv, c, lbr)
      in
  ...
  let rec check_rec_call_stack renv stack rs t =
      match kind t with
        ...
        | Case (ci, u, pms, ret, iv, c_0, br) -> (* iv ignored: it's just a cache *)
            let _, (p, _), _, _, brs =
              expand_case (lookup_mind_specif renv.env ci.ci_ind) (ci, u, pms, ret, iv, c_0, br)
            in
    ...
    if not (noccur_with_meta n nbfix t) then
      ...
      match kind c with
        ...
        | Case (ci, u, pms, p, iv, tm, br) -> (* iv ignored: just a cache *)
          begin
            let _, (p, _), _, _, vrest =
              expand_case (lookup_mind_specif env ci.ci_ind) (ci, u, pms, p, iv, tm, br)
            in
