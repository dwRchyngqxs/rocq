  ...
  | (CONSTRUCT(((sp, n), _), [||]), APP(args,CASE(u,pms,_p,br,iv,ci,env,stk)))
      ...
      let env =
        if (Int.equal ci.ci_cstr_ndecls.(n - 1) ci.ci_cstr_nargs.(n - 1)) then (* no lets *)
          ...
          let ctx = Environ.expand_branch_context info.env (sp, u) pms br (n - 1) in
...
and cbv_apply_rule info env ctx psubst es stk =
  ...
  | Declarations.PECase (pind, pu, pret, pbrs) :: e, CASE (u, pms, (p, _), brs, iv, ci, env, s) ->
      ...
      let ntys_ret, ntys_brs = Environ.expand_case_contexts info.env (ci.ci_ind, u) pms (fst p) brs in
...
let rec apply_stack info t = function
  ...
  | CASE (u, pms, (ty, tyr), br, iv, ci, env, st) ->
    (* FIXME: Prevent this expansion by caching whether an inductive contains let-bindings *)

    let tyctx, brctx = Environ.expand_case_contexts info.env (ci.ci_ind, u) pms (fst ty) br in
    let ty = tyctx, snd ty in
    ...
    in
      apply_stack info
        (mkCase (ci, u, Array.map (aux env) pms, (map_ctx ty, tyr), iv, t,
