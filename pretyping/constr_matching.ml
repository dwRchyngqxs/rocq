  ...
  let rec sorec ctx env subst (p:constr_pattern) t =
    ...
    match p, EConstr.kind sigma cT with
      ...
      | PIf (a1,b1,b1'), Case (ci, u2, pms2, p2, iv, a2, ([|_, b2; _, b2'|] as br2)) ->
          let b2ctx = EConstr.annotate_branches env sigma ci u2 pms2 br2 in
          let ctx_b2 = b2ctx.(0) in
          let ctx_b2' = b2ctx.(1) in
      ...
      | PCase (ci1, p1, a1, br1), Case (ci2, u2, pms2, p2, iv, a2, br2) ->
          let p2ctx, br2ctx = EConstr.annotate_case env sigma ci2 u2 pms2 p2 br2 in
...
let sub_match ?(closed=true) env sigma pat c =
  ...
  | Case (ci, u, pms, (hd0, hdr), iv, c1, lc0) ->
      let hdctx, brctx = annotate_case env sigma ci u pms (hd0, hdr) lc0 in
