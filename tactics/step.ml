...
| ZCaseB (ci, u, pms, p, iv, c, (n, brs)) ->
  mkCase (ci, u, pms, p, iv, c, let nas, br = Array.unsafe_get brs n in array_with brs n (nas, x))
...
| [| [||] , br |] ->
  ...
  let expect_indices =
    try snd (destApp (snd (Array.unsafe_get mip.mind_nf_lc 0)))
    with DestKO -> [||]
  in
  let nind = Array.length indices in
  let rec loop i =
    if Int.equal nind i then true else
    let expected = Array.unsafe_get expect_indices (mib.mind_nparams + i) in
    let expected = Vars.substl pms expected in
    match Conversion.conv env expected (Array.unsafe_get indices i) with
    | Ok () -> loop (i + 1)
    | Error () -> false
  in
...
let match_uip_red env ci u pms indices brs =
  let mip, mib = lookup_mind_specif env ci.ci_ind in
  let ps = get_match_param_context mip u pms in
  match_uip_red_specif env (mip, mib) u ps indices brs
  ...
  let rec aux c =
    ...
    | App (head, args) ->
      or_step (fun head -> mkApp (head, args)) (aux head)
        ( fun _ ->
          or_step (fun hd -> mkApp (head, array_with args 0 hd)) (aux (Array.unsafe_get args 0))
            ( fun _ ->
                  ...
                  or_step (fun brs -> mkCase (ci, u, pms, bi, iv, c, brs))
                    ...
                    ( fun _ ->
                      ...
                      Option.map (fun p -> mkCase (ci, u, pms, ((nas, p), rp), iv, c, brs))
                        ( reduce_or_normalize
                          (push_rel_context (expand_arity_specif oib (ci.ci_ind, u) ps nas))
                          p
                        )
                    )
                )
...
let global_step evm env c =
  let rec aux env c =
    ...
    | App (head, args) ->
      or_step (fun head -> mkApp (head, args)) (aux env head)
        ( fun _ ->
          or_step (fun hd -> mkApp (head, array_with args 0 hd)) (aux env (Array.unsafe_get args 0))
            ( fun _ ->
              or_else (app_head env head args)
            ...
            ( fun _ ->
              or_step (fun pms -> mkCase (ci, u, pms, bi, iv, c, brs)) (array_step (aux env) pms)
                ( fun _ ->
                  ...
                  or_else
                    ( match iv with
                      | CaseInvert {indices} -> match_uip_red_specif env (mib, oib) u ps indices brs
                      | _ -> None
                    )
                    ( fun _ ->
                      or_step
...
let step red env evm c =
  let f =
    ...
    | RHZetaMatch ((ind, n, m), occ) ->
      e_contextually occ (zeta_match_prep ind n m) (zeta_match_step n m)
