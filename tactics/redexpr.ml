      ...
      | Some ((ind, tyi), x) ->
        ...
        let n, m =
          ...
          | None ->
            ...
            1, 1
          | Some (n, None) -> if nbrs = 1 then 1, n else n, 1
          | Some (n, Some m) -> n, m
        in
        ...
        | LocalDef (_, c, _) :: t -> if m != 0 then bind_to_index (m - 1) (k + 1) t else k
        in
        (ind, tyi),
        n,
        bind_to_index m 0 (CList.firstn (mip.mind_consnrealdecls.(n - 1)) (fst mip.mind_nf_lc.(n - 1)))
    )
