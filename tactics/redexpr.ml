...
let check_custom = function
  ...
  | Step _ -> failwith "failure in check_custom"
  |_ -> ()
...
let bind_red_expr_occurrences occs nbcl redexp =
  ...
  let has_at_clause = function
    ...
    | Step _ -> failwith "failure in bind_red_expr_occurrences:has_at_clause"
    | _ -> false in
  if occs == AllOccurrences then
    ...
    | Unfold [] | Pattern [] ->
        assert false
    | Step _ -> failwith "failure in bind_red_expr_occurrences"
...
module Intern = struct
  ...
  let intern_red_expr ist = function
    | Step s -> Step (Step.map_reduction (intern_zeta ist) (intern_evaluable ist) s)
...
module Interp = struct
  ...
  let interp_zeta env (gr, x) =
    let zmargs =
      ...
      | ConstRef c, _ ->
        ( try
            let s = Structures.Structure.find_from_projection c in
            Some (s.name, x)
          with Not_found -> None
        )
  ...
  let interp_red_expr ist env sigma = function
    | Step s -> sigma, Step (Step.map_reduction (interp_zeta env) (interp_evaluable ist env sigma) s)
