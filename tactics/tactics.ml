...
let reduce redexp cl =
  ...
  (* WIP: depends on the reduction, for instance delta depends on named environment because of `pose`
    "introducing dependencies on variables":
      happens for instance when folding with a posed variable on an hypothesis above that variable
      (so should not happend in our case unless we introduce expansion steps)
  *)
  | Step r -> (match r with RHDelta _ | RHead | RCbv | RCbn | RLazy -> StableHypConv | _ -> LocalHypConv)
