...
Eval step root in (fun f => f (@id nat)) (id (@id (nat -> nat))) O.
...
Eval step root in let x := id O in x.
Eval step head in let x := id O in x.
Eval step cbv in let x := O in x.
(* Zeta_match x reduction *)
Eval step zeta_match "nvlbp" 1 in match id (vlbp nat) with vlbp _ x => id x end.
Eval step zeta_match NVLBP 1 in match id (vlbp nat) with vlbp _ x => id x end.
Eval step zeta_match VLBP 1 in match id (vlbp nat) with vlbp _ x => id x end.
...
Eval step root in match id vlb with vlb x => id x end.
...
Eval step root in match id (vlbp True) with vlbp _ x => id x end.
...
Eval step root in match id vlbt with vlbf x f => id f (id x) | vlbt x f => id f (id x) end.
Eval step root in match id vlbt with vlbf x f => id x | vlbt x f => id f (id x) end.
Eval step root in match id vlbt with vlbf x f => I | vlbt x f => id f (id x) end.
Eval step root in match id vlbt with vlbf x f => I | vlbt x f => id x end.
...
Check ltac:(let tm := eval step root in (fun T: Type@{PBox.u0} => PBox T) in exact tm).
Check ltac:(let tm := eval step root in (fun T: Type@{TfT.u0} => TfT T) in exact tm).
Check ltac:(let tm := eval step root in (fun T: Type@{TfS.u0} => TfS T) in exact tm).
...
Fail Eval step head in (fun T: Type@{PBox.u0} => PBox T).
Fail Eval step eta in (fun T => PBox T).
Fail Eval step eta in (fun T => TfT T).
Fail Eval step eta in (fun T => TfS T).
Fail Eval step root in (fun T => PBox T).
Fail Eval step root in (fun T => TfT T).
Fail Eval step root in (fun T => TfS T).
...
Eval step root in bpair (id epsilonbp).(pfst) (id epsilonbp).(psnd).
...
Eval step root in A epsilonra.(ttfst) epsilonra.(ttsnd).
...
Eval step root in B epsilonrb.(tttrd) epsilonrb.(ttfth).
...
Fail Eval step root in bpair (id epsilonbp).(psnd) (id epsilonbp).(pfst).
...
Fail Eval step root in bpair2 (id epsilonbp).(pfst) (id epsilonbp).(psnd).
...
Fail Eval step root in bpair (id epsilonbp).(pfst) (epsilonbp).(psnd).
...
Fail Eval step root in A epsilonrb.(tttrd) epsilonrb.(ttfth).
...
Fail Eval step root in B epsilonra.(ttfst) epsilonra.(ttsnd).
...
Fail Eval step root in C epsilonra.(ttfst).
...
Abort.
(* Fix reduction *)
Eval step iota_fix in (fix any (x: bool) := x) false.
Eval step root in (fix any (x: bool) := x) false.
Eval step head in (fix any (x: bool) := x) false.
Eval step cbv in (fix any (x: bool) := x) false.
Eval step iota_fix in (fix any T (x: bool) := x) (id bool) false.
Eval step root in (fix any T (x: bool) := x) (id bool) false.
Eval step head in (fix any T (x: bool) := x) bool false.
Eval step cbv in (fix any T (x: bool) := x) bool false.
Eval step iota_fix in (fix is_even x := match x with O => true | S n => is_odd n end with is_odd n := match n with O => false | S n => is_even n end for is_even) O.
Eval step root in (fix is_even x := match x with O => true | S n => is_odd n end with is_odd n := match n with O => false | S n => is_even n end for is_even) O.
...
Fail Eval step iota_fix in (fix any (x: bool) := x) (id false).
Fail Eval step root in (fix any (x: bool) := x) (id false).
Fail Eval step head in (fix any (x: bool) := x) epsilonb.
Fail Eval step cbv in (fix any (x: bool) := x) epsilonb.
Fail Eval step iota_fix in (fix any T (x: bool) := x) (id bool).
Fail Eval step root in (fix any T (x: bool) := x) (id bool).
Fail Eval step head in (fix any T (x: bool) := x) bool.
Fail Eval step cbv in (fix any T (x: bool) := x) bool.
(* Fix' reduction *)
Eval step iota_fix' in (fix any (x: bool) := x).
Eval step iota_fix' in (fix is_even x := match x with O => true | S n => is_odd n end with is_odd n := match n with O => false | S n => is_even n end for is_even).
(* Cofix reduction *)
Eval step iota_cofix in match (cofix x := Cons true x) with Cons h t => h end.
Eval step root in match (cofix x := Cons true x) with Cons h t => h end.
...
Eval step iota_cofix in match (cofix x := PCons true x) with {| PHead := x |} => x end.
Eval step root in match (cofix x := PCons true x) with {| PHead := x |} => x end.
...
Eval step iota_cofix in match cofix x := Cons true y with y := Cons false x for x with Cons h t => t end.
Eval step root in match cofix x := Cons true y with y := Cons false x for x with Cons h t => t end.
...
Eval step iota_cofix in match cofix x := PCons true y with y := PCons false x for x with {| PTail := x |} => x end.
Eval step root in match cofix x := PCons true y with y := PCons false x for x with {| PTail := x |} => x end.
...
Fail Eval step iota_cofix in (cofix x := PCons true x).(PHead).
Fail Eval step iota_cofix in cofix x := PCons true y with y := PCons false x for x.
Fail Eval step root in cofix x := PCons true y with y := PCons false x for x.
...
(* Cofix' reduction *)
Eval step iota_cofix' in cofix x := Cons true x.
Eval step iota_cofix' in cofix x := Cons true y with y := Cons false x for x.
(* Match reduction *)
Eval step iota_match in match I with I => id O end.
Eval step root in match I with I => id O end.
Eval step head in match I with I => id O end.
Eval step cbv in match I with I => id O end.
Eval step iota_match in match true with true => id false | false => id true end.
Eval step root in match true with true => id false | false => id true end.
...
Eval step iota_match in match false with true => id false | false => id true end.
Eval step root in match false with true => id false | false => id true end.
...
Eval step iota_match in match box nat (id O) with box _ x => id x end.
Eval step root in match box nat (id O) with box _ x => id x end.
Eval step head in match box nat (id O) with box _ x => id x end.
Eval step cbv in match box nat O with box _ x => id x end.
Eval step iota_match in match box (nat -> nat) (id S) with box _ f => id f (id O) end.
Eval step root in match box (nat -> nat) (id S) with box _ f => id f (id O) end.
...
Eval step iota_match in match rlb with rlb x => id x end.
Eval step root in match rlb with rlb x => id x end.
Eval step head in match rlb with rlb x => id x end.
Eval step cbv in match rlb with rlb x => id x end.
Eval step iota_match in match vlb with vlb x => id x end.
Eval step root in match vlb with vlb x => id x end.
Eval step head in match vlb with vlb x => id x end.
Eval step cbv in match vlb with vlb x => id x end.
Fail Eval step root in match epsilonvlb with vlb => id I end.
Fail Eval step head in match epsilonvlb with vlb => id I end.
Fail Eval step cbv in match epsilonvlb with vlb => I end.
Fail Eval step root in match id vlb with vlb => id I end.
(* Projection reduction *)
Eval step iota_match in match wrap (id I) with {| unwrap := x |} => x end.
Eval step root in match wrap (id I) with {| unwrap := x |} => x end.
Eval step head in match wrap (id I) with {| unwrap := x |} => x end.
Eval step cbv in match wrap I with {| unwrap := x |} => x end.
Eval step iota_match in match bpair (id true) (id false) with {| pfst := x |} => x end.
Eval step root in match bpair (id true) (id false) with {| pfst := x |} => x end.
...
Eval step iota_match in match bpair (id true) (id false) with {| psnd := x |} => x end.
Eval step root in match bpair (id true) (id false) with {| psnd := x |} => x end.
...
Eval step iota_match in match pbox (id nat) (id O) with {| punbox := x |} => x end.
Eval step root in match pbox (id nat) (id O) with {| punbox := x |} => x end.
...
Eval step iota_match in match (fun x => x) epsilonsu with sunit => id O end.
Eval step root in match (fun x => x) epsilonsu with sunit => id O end.
Eval step head in match (fun x => x) epsilonsu with sunit => id O end.
Eval step cbv in match epsilonsu with sunit => id O end.
Eval step iota_match in match (fun x => x) epsilonsnn with seq _ => id O end.
Eval step root in match (fun x => x) epsilonsnn with seq _ => id O end.
Eval step head in match (fun x => x) epsilonsnn with seq _ => id O end.
Eval step cbv in match epsilonsnn with seq _ => id O end.
Fail Eval step iota_match in match epsilon01 return nat with seq _ => id O end.
Fail Eval step root in match epsilon01 return nat with seq _ => id O end.
