...
Fail Eval step zeta_match vlbt 1 in match vlb with vlb x => x end.
Fail Eval step zeta_match vlbt in match vlb with vlb x => I end.
...
Check fun T => TfT T.
Check fun T => TfS T.
Check fun T: Set => PBox T.
Check fun T: Set => TfT T.
Check fun T: Set => TfS T.
(* TODO: Where does the new Type comes from? *)
...
(* TODO: Not sure this is correct, check if solved after making the next unit tests fail *)
Check ltac:(let tm := eval step eta' in (fun T => PBox T) in exact tm).
Check ltac:(let tm := eval step eta' in (fun T => TfT T) in exact tm).
Check ltac:(let tm := eval step eta' in (fun T => TfS T) in exact tm).
Check ltac:(let tm := eval step head in (fun T => PBox T) in exact tm).
Check ltac:(let tm := eval step head in (fun T => TfT T) in exact tm).
Check ltac:(let tm := eval step head in (fun T => TfS T) in exact tm).
Check ltac:(let tm := eval step cbv in (fun T => PBox T) in exact tm).
Check ltac:(let tm := eval step cbv in (fun T => TfT T) in exact tm).
Check ltac:(let tm := eval step cbv in (fun T => TfS T) in exact tm).
(* TODO: does not fail
Check ltac:(let tm := eval step eta' in (fun T: Set => PBox T) in exact tm).
Check ltac:(let tm := eval step eta' in (fun T: Set => TfT T) in exact tm).
Check ltac:(let tm := eval step eta' in (fun T: Set => TfS T) in exact tm).
...
Fail Eval step cofix in cofix x := PCons true y with y := PCons false x for x.
...
Fail Eval step match in match epsilonvlb with vlb => id I end.
Fail Eval step head in match epsilonvlb with vlb => id I end.
Fail Eval step cbv in match epsilonvlb with vlb => I end.
Fail Eval step match in match id vlb with vlb => id I end.
...
Eval step cbv in (fun x => Nat.add x) (id O).
Eval step cbv in (fun x y => Nat.add x y) (id O) (id O).
(* Reduce first arg under App *)
Eval step cbv in (fun x => x) ((fun x => x) O).
(* TODO: WRONG ORDER *)
...
(* Reduce body under of Proj *)
Eval step cbv in (wrap ((fun x => x) I)).(unwrap).
...
(* TODO: succeeds
Eval lazy in match epsilon01 return nat with seq _ => id O end. (* Fails *)
